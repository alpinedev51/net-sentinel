...
class AuditBase(BaseModel):
    ...
    name: str = Field(
        ..., max_length=255, description="The name of the audit campaign."
    )
...
class AuditCreate(AuditBase):
    ...
    target_ids: list[int] = Field(
        ..., min_length=1, description="List of target IDs to scan."
    )
    tools: list[int] = Field(
        ..., min_length=1, description="List of tools to use (e.g., nmap, ping)."
    )
